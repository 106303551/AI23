...
def joinFactors(factors: List[Factor]):
    ...
    "*** YOUR CODE HERE ***"
    # uncondiction condition 分兩類
    # 如果condition 出現在 uncondiction 丟到 uncondiction
    ...
    for factor in factors:
        ...
        for var in unconvars:
            ...
            if var not in inputUnconditionedVariables:
                inputUnconditionedVariables.append(var)
        for var in convars:
            if var not in inputUnconditionedVariables:
                if var not in inputConditionedVariables:
                    inputConditionedVariables.append(var)
        vardomaindict = factor.variableDomainsDict()

        for k,v in vardomaindict.items():
            inputVariableDomainsDict[k] = v

    newfactor = Factor(inputUnconditionedVariables, inputConditionedVariables , inputVariableDomainsDict)
    ...
    for assignment in assignments:
        ...
        for factor in factors:
            ...
            for oldAssignment in oldAssignments:
                trueFlag = True
                for k,v in oldAssignment.items():
                    if assignment[k] != v:
                        trueFlag = False
                        break
                if trueFlag:
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        for assignment in assignments:
            ...
            prob += newFactor.getProbability(assignment)
            newFactor.setProbability(assignment,prob)
        return newFactor
        "*** END YOUR CODE HERE ***"
