...
def sentence1() -> Expr:
    ...
    senten_1 = A|B
    senten_2 = ~A%(~B|C)
    senten_3 = disjoin(~A,~B,C)
    return conjoin(senten_1,senten_2,senten_3)
    "*** END YOUR CODE HERE ***"
...
def sentence2() -> Expr:
    ...
    return conjoin(sentence1,sentence2,sentence3,sentence4)
...
def sentence3() -> Expr:
    ...
    l1 = PacmanAlive_1 % ((PacmanAlive_0&~PacmanKilled_0)|(~PacmanAlive_0&PacmanBorn_0))
    l2 = ~(PacmanAlive_0&PacmanBorn_0)
    l3 = PacmanBorn_0
    return conjoin(l1,l2,l3)
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    class testingclass:
        def __init__(self,var : str = 'a'):
            self.var = var
        def __repr__(self):
            return self.var
    sol = {testingclass('a'):True}
    return sol
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    flag = findModel(premise & ~conclusion)
    return flag == False
    "*** END YOUR CODE HERE ***"
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    flag = pl_true(~inverse_statement,assignments)
    return flag
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    #最多一為真 兩兩找組合 對組合取not並disjoin 如果有兩個以上Expr為真 就會有一個組合為False 最後將取完not並disjoin的結果保存然後conjoin 就能驗證是否最多一個
    combine_2 = itertools.combinations(literals,2)
    pairs = []
    for c in combine_2:
        pairs.append(disjoin(~c[0],~c[1]))
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    #剛好一個為真 atLeastOne與atMostOne的交集
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #找出為何能走到(x,y)在t
    print(PropSymbolExpr(pacman_str, x, y, time=now) % disjoin(possible_causes))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #Add KB
    #1.pacphysics_axioms(...) with the appropriate timesteps. There is no sensorModel because we know everything about the world. Where needed,
    #use allLegalSuccessorAxioms for transitions since this is for regular Pacman transition rules.
    #確認 pacman location
    pac_axioms = pacphysicsAxioms(0,all_coords,non_outer_wall_coords,walls_grid,None,allLegalSuccessorAxioms)
    KB.append(pac_axioms)
    #確認 potential location
    pot_axioms = pacphysicsAxioms(1,all_coords,non_outer_wall_coords,walls_grid,None,allLegalSuccessorAxioms)
    KB.append(pot_axioms)
    #加入pacman位置到KB Pacman is at (x,y) at time t is PropSymbolExpr(pacman_str, x, y, time=t)
    KB.append(PropSymbolExpr(pacman_str,x0,y0,time=0))
    #加入　action0 action is taken at t is PropSymbolExpr(action, time=t).
    KB.append(PropSymbolExpr(action0,time=0))
    #加入　action1 action is taken at t is PropSymbolExpr(action, time=t).
    KB.append(PropSymbolExpr(action1,time=1))

    KB_expr = conjoin(KB)
    loc_expr = PropSymbolExpr(pacman_str,x1,y1,time=1)

    return findModel(KB_expr & loc_expr),findModel(KB_expr & ~loc_expr)
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x,y in non_wall_coords:
            pacmans.append(PropSymbolExpr(pacman_str,x,y,time=t))
        KB.append(exactlyOne(pacmans))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        #完成條件是全吃完
        if findModel(conjoin(KB) & ~atLeastOne(foods)) != False:
            return extractActionSequence(findModel(conjoin(KB) & ~atLeastOne(foods)),actions)
